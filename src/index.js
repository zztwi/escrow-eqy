...
function feeFor(amount, percent = config.feePercent) {
  return Math.round((amount * percent / 100) * 100) / 100;
}

function feePercentForTrade(trade) {
  return trade.fee_percent || config.feePercent;
}

function feePercentForMember(member) {
  return hasRole(member, config.boosterRewardRoleId) ? config.boosterFeePercent : config.feePercent;
}
...
function compactTradeEmbed(trade, title = "eqy escrow") {
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: "Fee", value: `${money(trade.fee, trade.currency)} (${feePercentForTrade(trade)}%)`, inline: true }
    )
...
function ticketIntro(trade) {
  ...
  return [
    ...
    `Fee: ${money(trade.fee, trade.currency)} (${feePercentForTrade(trade)}%)`,
    "",
    "**Workflow**",
...
function boostRewardsText() {
  ...
  return [
    ...
    "**Booster perks**",
    `- ${rewardRole}`,
    "- priority visibility when support queues are busy",
    "- access to the private booster channel",
    `- reduced escrow fee: ${config.boosterFeePercent}% instead of ${config.feePercent}%`,
...
async function handleStart(interaction) {
  ...
  const appliedFeePercent = feePercentForMember(interaction.member);
  const trade = db.createTrade({
    ...
    amount,
    fee: feeFor(amount, appliedFeePercent),
...
function boostDmText() {
  return [
    ...
    "- access to the private booster channel",
    `- reduced escrow fee: ${config.boosterFeePercent}% instead of ${config.feePercent}%`,
    "- access to monthly booster-only giveaways",
    "",
    "Useful links:",
