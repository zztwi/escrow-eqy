...
function boostRewardsText() {
  ...
  return [
    "# eqy boost rewards",
    "",
    "Boost the server to support cleaner escrow tickets, faster support visibility, and a safer PayPal trading community.",
    "",
    "**Booster perks**",
    `- ${rewardRole}`,
    "- priority visibility in support queues",
    "- premium supporter badge inside the community",
    "- access to future booster-only drops, perks, and partner benefits",
...
function boostThanksText(member) {
  const rewardLine = config.boosterRewardRoleId ? `Unlocked: <@&${config.boosterRewardRoleId}>` : "Unlocked: booster perks pending staff setup";
  return [
    ...
    rewardLine,
    "",
    "Your boost helps us keep PayPal escrow tickets cleaner, support faster, and trade history easier to review.",
    "",
    "Reminder: keep every trade inside tickets and always save PayPal references, screenshots, and proof."
  ].join("\n");
}
...
client.on("guildMemberUpdate", async (oldMember, newMember) => {
  ...
  if (startedBoosting) {
    if (config.boosterRewardRoleId) {
      await newMember.roles.add(config.boosterRewardRoleId, "eqy server boost reward").catch((error) => {
        console.error(`Failed to assign booster reward role ${config.boosterRewardRoleId} to ${newMember.user.tag}:`, error);
      });
    }

    const channelId = config.boostLogChannelId || config.welcomeChannelId || newMember.guild.systemChannelId || config.infoChannelId;
    const channel = await newMember.guild.channels.fetch(channelId).catch(() => null);
    if (channel) {
      await channel.send(boostThanksText(newMember)).catch((error) => {
        console.error(`Failed to send boost reward message in ${channelId}:`, error);
      });
    }
    ...
    await newMember.send([
      ...
      "",
      "Your premium supporter reward is now active.",
      config.boosterRewardRoleId ? `Unlocked role: <@&${config.boosterRewardRoleId}>` : "Staff will finish booster reward setup soon.",
      "",
      "Your boost helps keep the server cleaner, support more responsive, and trades easier to protect.",
      "",
      "Reminder: escrow safety still depends on ticket IDs, PayPal references, screenshots, and proof."
    ].join("\n")).catch(() => null);
  }
