...
function compactTradeEmbed(trade, title = "eqy escrow") {
  return new EmbedBuilder()
    .setColor(embedColor)
    .setTitle(`${title} - ${ticketId(trade)}`)
    .addFields(
      { name: "Status", value: statusLabels[trade.status] || trade.status, inline: true },
      { name: "Amount", value: money(trade.amount, trade.currency), inline: true },
      ...
      { name: "Total", value: money(trade.amount + trade.fee, trade.currency), inline: true }
    )
    .setFooter({ text: "eqy escrow - PayPal manual verification" })
...
function logEmbed(title, trade, actor, extra = []) {
  const embed = compactTradeEmbed(trade, title)
    .setDescription(actor ? `Action by <@${actor.id}>` : null);
...
function ticketIntro(trade) {
  ...
  return [
    ...
    "",
    `**eqy escrow ticket:** ${ticketId(trade)}`,
    `**Item/service:** ${trade.description}`,
    `**Buyer pays:** ${money(total, trade.currency)} (${money(trade.amount, trade.currency)} + ${config.feePercent}% fee)`,
    `**Seller receives:** ${money(trade.amount, trade.currency)}`,
    "",
    "**Flow**",
    `1. Buyer sends PayPal payment to: ${config.paypalVaultEmail}`,
    "2. Buyer clicks **Submit deposit** and enters the PayPal transaction ID.",
    "3. Buyer can click **Add proof** and upload a screenshot.",
    "4. Staff verifies PayPal with commands.",
    "5. Seller delivers.",
    "6. Buyer clicks **Release**.",
    "",
    "**Rules**",
    "The service fee is charged once, funds are not invested, and disputes are decided by staff."
  ].join("\n");
}
...
function helpText(trade) {
  ...
  return [
    `**eqy commands - ${id}**`,
    `Buyer button: Submit deposit`,
    `Buyer button: Release`,
    `Buyer/Seller button: Dispute`,
    `Buyer/Seller button: Add proof`,
    `Buyer/Seller button: Review`,
...
function reportPanelText() {
  return [
    "**eqy reports**",
    "",
    "Open a ticket if you need to report a scam, suspicious user, fake proof, unpaid deal, or abuse related to an escrow.",
    "",
    "**Before opening**",
    "Have screenshots, PayPal references, Discord IDs, ticket IDs, and a short explanation ready.",
    "",
    "Reports without proof may be closed."
  ].join("\n");
}
...
function partnershipPanelText() {
  return [
    "**eqy partnerships**",
    "",
    "Open a ticket if you want to partner with eqy.",
    "",
    "Prepare your server invite, member count, niche, reason, and what you can offer.",
    "",
...
function botInfoText() {
  return [
    "**eqy escrow bot**",
    "",
    "eqy helps two users trade through a PayPal escrow workflow.",
    "",
    "**What it does**",
    "- creates private escrow threads",
    "- calculates an 8% service fee",
    "- tracks PayPal deposit and payout references",
    "- stores proof in the proof channel",
    "- lets users open disputes and reports",
    "- keeps staff-only actions command based",
    "",
    "**User flow**",
    "1. Use `/escrow start` in the escrow start channel.",
    "2. Buyer pays PayPal and clicks `Submit deposit`.",
    "3. Buyer uploads proof when requested.",
    "4. Staff confirms the deposit.",
    "5. Seller delivers.",
    "6. Buyer clicks `Release`.",
    "7. Staff confirms payout.",
    "",
    `For reports or support, use the button in <#${config.reportPanelChannelId}>. Always include proof and the ticket ID.`
  ].join("\n");
}
...
async function submitDeposit(interaction, id, txId) {
  ...
  await sendLog(interaction.guild, config.depositLogChannelId, {
    ...
    embeds: [
      logEmbed("PayPal deposit submitted", updated, interaction.user, [
        { name: "PayPal TX", value: shortId(txId), inline: true },
        { name: "Next", value: "Verify PayPal, then use `/escrow confirm_deposit`." }
      ])
    ]
  });
  ...
  await interaction.reply({
    content: [
      ...
      "If you have a screenshot, send it in this ticket now. I will repost it cleanly as eqy proof."
    ].join("\n"),
...
async function handleDisputeAdmin(interaction) {
  ...
  await sendLog(interaction.guild, config.disputeLogChannelId, {
    content: ["**dispute update**", `Ticket ID: ${ticketId(updated)}`, `Status: ${status}`, winner ? `Winner: ${winner}` : null, `Staff: <@${interaction.user.id}>`, `Reason: ${reason}`].filter(Boolean).join("\n")
  });
  await interaction.reply({ content: `${ticketId(updated)} marked as ${status}.` });
}
...
async function postToChannel(interaction, channelId, content, components = []) {
  if (!isModerator(interaction)) {
    await denyStaffAction(interaction, interaction.commandName);
    return;
  }
...
function welcomeText(member) {
  return [
    ...
    "",
    "**eqy quick start**",
    `Escrow: go to <#${config.startChannelId}> and use \`/escrow start\`.`,
    `Reports/support: open a ticket in <#${config.reportPanelChannelId}>.`,
    `Verification: verify in <#${config.verifyChannelId}>.`,
...
client.on("guildMemberAdd", async (member) => {
  if (config.memberRoleId) {
    await member.roles.add(config.memberRoleId, "eqy auto member role").catch(console.error);
  }

  const channelId = config.welcomeChannelId || config.infoChannelId;
  const channel = await member.guild.channels.fetch(channelId).catch(() => null);
  if (channel) {
    await channel.send(welcomeText(member)).catch(console.error);
  }
});
